...
- [x] `IElementContribution` 定义完整(4 个方法签名)
- [x] Spring1D/Truss2D/EulerBeam2D 三个 Contribution 类全部实现接口
- [x] `ElementContributionRegistry` 注册与分派
- [x] `Assembler.cs` 刚度与荷载装配改为单循环 registry dispatch
- [x] `DofLayout.Build` 自由度收集改为 registry dispatch
- [x] `LinearStaticAnalysis.cs` 内力恢复改为 registry dispatch
- [x] `dotnet test` **15/15 全绿**(接口重构不改变计算结果,新增 2 个单位制测试)
- [x] 代码注释标注"P1.v0.1b 接口化完成,为 P2 Track A 预留插件位"
...
- [x] `Tests/Unit/UnitSystemConsistencyTests.cs` 创建,包含 2 个测试用例
- [x] 测试1:悬臂梁均布荷载 SI↔MmN 一致性验证
- [x] 测试2:简支梁中点集中力 SI↔MmN 一致性验证
- [x] `dotnet test` 新增测试通过(13→15,全部通过)
- [x] 测试注释说明"当前 Units 为元数据,一致性由调用方保证几何/材料换算"
...
- [x] `BeamMvpPanel.xaml` 增加 EndA/EndB ComboBox 控件
- [x] ViewModel 暴露 `SupportOptions` 枚举列表
- [x] 面板可选 Free/Pin/Fixed,改变后点击"求解并回写"生效
- [x] HyCADTool 项目编译成功,无警告无错误
- [ ] 验证简支(Pin-Pin)、两端固接(Fixed-Fixed)与 G07/G08/G09 解析解一致(需在 AutoCAD 中实测)
...
| 2026-07-10 | v1.1 | **完成实施**:P1.v0.1b 接口化(IElementContribution+Registry,6个文件改造) / P1.v0.3 单位制测试(2个用例) / P1.v0.2 UI补齐(EndA/EndB ComboBox);测试 13→15 全绿;更新验收标准为已完成 |
