                ...
                using (Transaction tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    foreach (ObjectId id in ms)
                    {
                        ...
                        foreach (var prop in props)
                        {
                            ...
                            if (propType != typeof(int) && propType != typeof(double) && propType != typeof(Point3d))
                                continue;

                            try
                            {
                                var val = prop.GetValue(ent); // 触发异常的检测
                                if (val != null)
                                {
                                    entityPropertyMap[entityType].Add((prop.Name, propType.Name));
                                }
                            }
                            catch
                            {
