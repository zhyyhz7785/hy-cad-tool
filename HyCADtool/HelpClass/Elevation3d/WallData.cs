...
namespace HyCADTool.HelpClass.CreatBase
{
    public partial class ElevationModelGenerator
    {
        ...
        )
        {
            ...
            {
                ed.WriteMessage("\n警告: 未指定 AnchorBoltType，跳过螺栓开孔生成\n");
                return;
            }
            ...
            double diameter = anchorBoltType.Diameter;
            double holeDepth = anchorBoltType.Depth + 150.0; // h1 + 150
        ...
        private void GenerateBasePlate(GeometryData geomData, BlockTableRecord btr, Transaction tr, double raftThickness)
        {
            ...
            // 生成螺栓孔（仅在底板上）
            GenerateBoltHoles(geomData, btr, tr, new AnchorBoltType("Default", 20.0, 300.0)); // 示例默认值，可根据需要调整
        }
        // 拉伸区域生成3D模型
