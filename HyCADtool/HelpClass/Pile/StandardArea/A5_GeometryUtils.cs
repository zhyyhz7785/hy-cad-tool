...
//            Coordinate p0 = vertices[0], p1 = vertices[1], p3 = vertices[3];
//            double lX = p1.X - p0.X;
//            double lY = p3.Y - p0.Y;
//            if (lX <= 0 || lY <= 0)
//            {
...
//                nY = (int)((d % 1 <= pileArrangeRate) ? Math.Floor(d) : Math.Ceiling(d));
//                nY = Math.Max(1, nY);
...
//                nX = (int)((d % 1 <= pileArrangeRate) ? Math.Floor(d) : Math.Ceiling(d));
//                nX = Math.Max(1, nX);
...
//        /// 计算梅花形布置的网格尺寸（统一输入参数）
//        /// </summary>
//        public static void CalculateGridSizeCircular(StandardAreaBase standardArea, Pile pile, double pileArrangeRate, ICadService cadService, out int nX, out int nY)
//        {
//            var contour = standardArea.Contour;
//            var inputDisplacementRate = standardArea.InputDisplacementRate;
...
//                return;
//            }
//            double pileArea = pile.PileArea; // 从 Pile 对象获取桩面积
//            double smallRectArea = pileArea / inputDisplacementRate;
//            double ss = Math.Sqrt(smallRectArea);
//            double totalPiles = Math.Ceiling(contour.Area / smallRectArea);
//            Coordinate[] vertices = contour.ExteriorRing.Coordinates;
//            Coordinate p0 = vertices[0], p1 = vertices[1], p3 = vertices[3];
//            double lX = p1.X - p0.X;
//            double lY = p3.Y - p0.Y;
