...
namespace HyCADTool.HelpClass.Jig
{
    public class HookJigSeg : EntityJig
    {
        ...
        private double boundarySegAngle;
        public HookJigSeg(Polyline polyline, double hookLength, LineSegment3d seg) : base(polyline)
        {
            ...
            Vector3d xAxis = new Vector3d(1, 0, 0); // X 轴方向向量
            boundarySegAngle = boundarySegDirection.GetAngleTo(xAxis);
        }
        ...
        protected override bool Update()
        {
            ...
            if (cross < 0)
            {
                angle = boundarySegAngle + Math.PI / 2;
            }
            else
            {
                angle = boundarySegAngle - Math.PI / 2;
