...
namespace HyCADTool
{
    public static partial class TestFunction
    {
        ...
        // 示例用法
        [CommandMethod("GroupTextByProximityAndCreatePolylines")]
        public static void GroupTextByProximityAndCreatePolylines()
        {
            Document doc = Application.DocumentManager.MdiActiveDocument;
            Editor ed = doc.Editor;
            Database db = doc.Database;
            ...
            Dictionary<DBText, ObjectId> inputDict = new Dictionary<DBText, ObjectId>();
            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                foreach (SelectedObject selObj in selSet)
                {
                    DBText text = trans.GetObject(selObj.ObjectId, OpenMode.ForRead) as DBText;
                    if (text != null)
                    {
                        inputDict.Add(text, selObj.ObjectId);
                    }
                }
                trans.Commit();
            }
            // 调用分组方法并创建多边形
            double proximityThreshold = 50.0; // 定义空间邻近性阈值，可以根据需要调整
            GroupBySpatialProximity(inputDict, proximityThreshold);
        }
    }
}
