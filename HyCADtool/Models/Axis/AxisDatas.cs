    ...
    public class AxisDatas
    {
        ...
        private AxisDatas(IEnumerable<Line> lines, List<Point3d> allPoints, double scale)
        {
            ...
            for (int i = 0; i < XAxes.Count; i++)
            {
                var ax = XAxes[i];
                ax.SerialNumber = i + 1;
                ax.Name = GetAlphabeticLabel(i);  // 使用字母标注
            ...
            for (int j = 0; j < YAxes.Count; j++)
            {
                var ay = YAxes[j];
                ay.SerialNumber = j + 1;
                ay.Name = ay.SerialNumber.ToString();  // 使用数字标注
            ...
            foreach (var p in pts)
            {
                ...
                if (best.HasValue)
                {
                    if (!map.ContainsKey(best.Value))
                    {
                        ...
                        var yAxis = YAxes.FirstOrDefault(a => a.Name == yName);  // y向轴线使用数字
                        var xAxis = XAxes.FirstOrDefault(a => a.Name == xName);  // x向轴线使用字母
