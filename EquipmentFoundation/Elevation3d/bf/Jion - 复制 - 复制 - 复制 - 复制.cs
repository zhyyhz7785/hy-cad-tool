...
//            }
//        }
//        public static void HandleWallConnection(Line q1, Line q2, Line q3, double thickness1, double thickness2, double thickness3,
//    ref Polyline qb1, ref Polyline qb2, ref Polyline qb3, WallData currentWall, List<WallData> allWalls, int currentIndex, out bool skipConnection)
//        {
//            // 在方法顶部统一声明变量
...
//            Vector3d dir3 = q3.EndPoint - q3.StartPoint;
//            // 计算 q1 和 q2 的夹角（弧度）
//            double angle12 = dir1.GetAngleTo(dir2);
//            bool isCollinear12 = Math.Abs(angle12) < 1e-6 || Math.Abs(angle12 - Math.PI) < 1e-6;
//            // 调试信息（中文输出，分行显示）
//            var doc = Application.DocumentManager.MdiActiveDocument;
//            var ed = doc.Editor;
//            ed.WriteMessage(
//                $"\n是否共线 (q1-q2): {isCollinear12}\n" +
//                $"存在重合边: {currentWall.Boundary.CoincidentEdge != null}\n" +
//                $"跳过条件: {ShouldSkipConnection(currentWall)}\n" +
//                $"墙体索引: {currentIndex}\n" +
//                $"q1: 起点({q1.StartPoint.X:F2}, {q1.StartPoint.Y:F2}) - 终点({q1.EndPoint.X:F2}, {q1.EndPoint.Y:F2}), 厚度: {thickness1:F2}\n" +
//                $"q2: 起点({q2.StartPoint.X:F2}, {q2.StartPoint.Y:F2}) - 终点({q2.EndPoint.X:F2}, {q2.EndPoint.Y:F2}), 厚度: {thickness2:F2}\n" +
//                $"q3: 起点({q3.StartPoint.X:F2}, {q3.StartPoint.Y:F2}) - 终点({q3.EndPoint.X:F2}, {q3.EndPoint.Y:F2}), 厚度: {thickness3:F2}\n"
//            );
//            // 判断是否跳过连接
//            skipConnection = ShouldSkipConnection(currentWall);
//            if (skipConnection)
//            {
//                ed.WriteMessage("\n为跳过的墙体生成调整后的缓冲区（q2 设置为空）。\n");
//                // 设置 q2 的缓冲区为空
//                qb2 = null; // 明确将 qb2 设置为 null，表示不生成图形
//                // 计算 q1 的缓冲区并调整
...
//                p22 = q2.StartPoint + normal2 * thickness2;
//                intersection = GetIntersectionPointNullable(p13, p13 + dir1, p22, p22 - dir2);
//                p13 = intersection.HasValue ? intersection.Value : p13;
//                qb1 = CreateBufferPolyline(p11, p12, p13, p14);
//                // 计算 q3 的缓冲区并调整
//                normal3 = dir3.GetNormal().RotateBy(Math.PI / 2, Vector3d.ZAxis);
//                p31 = q3.StartPoint;
//                p34 = q3.EndPoint;
//                p32 = p31 + normal3 * thickness3;
//                p33 = p34 + normal3 * thickness3;
//                p23 = q2.EndPoint + normal2 * thickness2;
//                intersection = GetIntersectionPointNullable(p32, p32 - dir3, p23, p23 + dir2);
//                p32 = intersection.HasValue ? intersection.Value : p32;
//                qb3 = CreateBufferPolyline(p31, p32, p33, p34);
//                ed.WriteMessage($"\nq1 调整后的 p13: {p13}\nq3 调整后的 p32: {p32}\n");
//                return;
//            }
//            // 非跳过情况，生成初始缓冲区
//            qb1 = GenerateInitialBuffer(q1, thickness1);
//            qb2 = GenerateInitialBuffer(q2, thickness2);
//            qb3 = GenerateInitialBuffer(q3, thickness3);
//            // 计算 q1 和 q2 的连接
//            normal1 = dir1.GetNormal().RotateBy(Math.PI / 2, Vector3d.ZAxis);
//            normal2 = dir2.GetNormal().RotateBy(Math.PI / 2, Vector3d.ZAxis);
//            p14 = q1.EndPoint;
...
//            double connectionAngle12 = CalculateAngleBetweenWalls(q1, q2);
//            bool isClockwise12 = connectionAngle12 >= 0;
//            if (isClockwise12)
//            {
...
//            normal3 = dir3.GetNormal().RotateBy(Math.PI / 2, Vector3d.ZAxis);
//            p23 = q2.EndPoint + normal2 * thickness2;
//            p32 = q3.StartPoint + normal3 * thickness3;
//            Point3d? j2Clockwise = GetIntersectionPointNullable(p23, p23 + dir2, p32, p32 - dir3);
//            double connectionAngle23 = CalculateAngleBetweenWalls(q2, q3);
//            bool isClockwise23 = connectionAngle23 >= 0;
//            if (isClockwise23)
//            {
//                if (thickness2 == thickness3)
//                {
//                    HandleSameThicknessClockwise(q2.EndPoint, p23, p32, normal2, normal3, thickness2, j2Clockwise, ref qb2, ref qb3);
//                }
//                else
//                {
//                    HandleDifferentThicknessClockwise(thickness2, thickness3, q2.EndPoint, p23, p32, j2Clockwise, null, ref qb2, ref qb3);
//                }
//            }
//            else
//            {
//                if (thickness2 == thickness3)
//                {
//                    HandleSameThicknessCounterClockwise(q2.EndPoint, p23, p32, normal2, normal3, thickness2, j2Clockwise, ref qb2, ref qb3);
//                }
//                else
//                {
//                    HandleDifferentThicknessCounterClockwise(q2.EndPoint, p23, p32, ref qb2, ref qb3);
//                }
//            }
//        }
//        // 已有的 ShouldSkipConnection 方法
...
//        }
//        // 已有的 GenerateInitialBuffer 方法
//        private static Polyline GenerateInitialBuffer(Line edge, double thickness)
//        {
//            Point3d p1, p2, p3, p4;
//            Vector3d dir = edge.EndPoint - edge.StartPoint;
//            Vector3d normal = dir.GetNormal().RotateBy(Math.PI / 2, Vector3d.ZAxis);
//            p1 = edge.StartPoint;
//            p4 = edge.EndPoint;
//            p2 = p1 + normal * thickness;
//            p3 = p4 + normal * thickness;
//            return CreateBufferPolyline(p1, p2, p3, p4);
//        }
//        // 共线墙体的处理方法（预留）
...
//                    result.LayerId = ElevationModelGenerator.BufferExId;
