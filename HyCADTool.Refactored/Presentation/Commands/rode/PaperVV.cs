...
[assembly: CommandClass(typeof(HyCADTool.Refactored.Presentation.Commands.PaperVV))]
namespace HyCADTool.Refactored.Presentation.Commands
{
    public class PaperVV
    {
        [CommandMethod("pvv", CommandFlags.Modal | CommandFlags.UsePickSet | CommandFlags.Redraw)]
                ...
                using (Transaction acTrans = db.TransactionManager.StartTransaction())
                {
                    ...
                    for (int i = 0; i < 15; i++)
                    {
                        Viewport acVport = new Viewport();
                            acVport.CenterPoint = new Point3d(0, 0, 0) + new Vector3d(0, 440 * i, 0);
                            acVport.Width = 240;
                            acVport.Height = 315;
                            acVport.CustomScale = 2;
                            acVport.SetUcs(Point3d.Origin, new Vector3d(0, 1, 0), new Vector3d(-1, 0, 0));
                            acVport.ViewCenter = point2d + new Vector2d(0, 210 * i - (420 - 315 + 10) / 2 * i);
