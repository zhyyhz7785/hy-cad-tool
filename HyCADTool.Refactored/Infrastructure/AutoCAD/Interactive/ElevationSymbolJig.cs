...
namespace HyCADTool.Refactored.Infrastructure.AutoCAD.Interactive
{
    ...
    /// 标高计算：(currentPoint.Y - basePoint.Y) / 1000.0
    /// </summary>
    public class ElevationSymbolJig : DrawJig
    {
        ...
        public void UpdateSymbol(double elevation, bool isBasePoint = false)
        {
            ...
            var textPos = RotatePoint(
                new Point3d(_currentPoint.X + (4 - sqrt2 / 2) * Scale, _currentPoint.Y + sqrt2 * Scale, 0),
                _currentPoint, _angleRadians);

            var vm = Presentation.ViewModels.SettingsPanelViewModel.Current;
            double textSize = (vm != null ? vm.TextSize : 2.5) * Scale;
        ...
        private void AdjustTextByState(double sqrt2)
        {
            ...
            {
                case ElevationSymbolState.FlipVertical:
                    textPos = new Point3d(textPos.X, _currentPoint.Y - (sqrt2 + textSize) * Scale, 0);
                    break;
                case ElevationSymbolState.FlipHorizontal:
                    textPos = new Point3d(_currentPoint.X - (4 - sqrt2 / 2) * Scale, textPos.Y, 0);
                    break;
                case ElevationSymbolState.FlipBoth:
                    textPos = new Point3d(_currentPoint.X - (4 - sqrt2 / 2) * Scale, _currentPoint.Y - (sqrt2 + textSize) * Scale, 0);
                    break;
            }
