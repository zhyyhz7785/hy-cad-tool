...
namespace HyCADTool.Refactored.Infrastructure.AutoCAD.Services
{
    ...
    public class DCELRenderer : IDCELRenderer
    {
        ...
        private CurveSegment2D FindMatchingCurveSegment(
            Domain.ValueObjects.Geometry.Point2D start,
            Domain.ValueObjects.Geometry.Point2D end,
            List<CurveSegment2D> curveSegments,
            double tolerance,
            out bool isReversed)
        {
        ...
        private CurveSegment2D CreateReversedSegment(CurveSegment2D segment)
        {
            ...
            else if (segment.Type == CurveSegmentType.Line && segment.OriginalLine.HasValue)
            {
                ...
                var reversedLine = new Domain.ValueObjects.Geometry.Line2D(line.EndPoint, line.StartPoint);
        ...
        private static bool IsPointOnSegment(
            Domain.ValueObjects.Geometry.Point2D p,
            Domain.ValueObjects.Geometry.Point2D a,
            Domain.ValueObjects.Geometry.Point2D b,
            double tol)
        {
            // 向量叉积判断共线
