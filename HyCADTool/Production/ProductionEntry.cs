...
// =============================================================================
//
// 触发：HyCADTool.csproj 的 Production PropertyGroup 定义了 HYCAD_PRODUCTION 常量。
//
// 作用：把 HyCADTool.dll 变成可以被 AutoCAD 直接 NETLOAD 的"成品插件 dll"，
//       不再依赖 ReCall.dll 这层热重载壳，最终客户安装包只需要：
//         - HyCADTool.dll（含本文件激活的两条 [assembly:] attribute）
//         - HyCADTool.dll 的 NuGet/项目依赖（HyCAD.BlenderUI.dll、Newtonsoft.Json.dll、Autofac.dll …）
...
[assembly: ExtensionApplication(typeof(HyCADTool.Production.ProductionExtension))]
[assembly: CommandClass(typeof(HyCADTool.Production.ProductionCommandFacade))]

namespace HyCADTool.Production
{
    /// <summary>
    /// 生产模式下的 AutoCAD 扩展应用入口。
    /// AutoCAD NETLOAD HyCADTool.dll 后会自动调用 Initialize / Terminate。
    /// 内部直接走 <see cref="HyCADTool.Shared.Bootstrap.PluginInitializer"/>
    /// 与开发模式中 ReCall 反射调用的入口完全一致 → 业务初始化路径 100% 复用。
    /// </summary>
    public sealed class ProductionExtension : IExtensionApplication
    {
        private static HyCADTool.Shared.Bootstrap.PluginInitializer _initializer;

        public void Initialize()
        {
            try
            {
                _initializer = new HyCADTool.Shared.Bootstrap.PluginInitializer();
