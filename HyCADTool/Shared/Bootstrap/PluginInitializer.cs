    ...
    public class PluginInitializer
    {
        ...
        private void InitializeRibbonAndMenus()
        {
            ...
            else
            {
                try
                {
                    HyCADTool.Shared.AutoCAD.UI.HyCadRibbonBuilder.Build();
            ...
            try
            {
                _cuixLoaded = HyCADTool.Shared.AutoCAD.UI.CuiMenuBuilder.EnsureLoaded();
        ...
        private void WriteEntryGuide()
        {
            ...
            if (_cuixLoaded)
            {
                if (HyCADTool.Shared.AutoCAD.UI.CuiMenuBuilder.IsMenuBarHidden())
        ...
        private void TerminateRibbonAndMenus()
        {
            try
            {
                if (string.Equals(Environment.GetEnvironmentVariable("HYCAD_RIBBON_FORCE_TEARDOWN"), "1", StringComparison.Ordinal))
                    HyCADTool.Shared.AutoCAD.UI.HyCadRibbonBuilder.Teardown();
                else
                    HyCADTool.Shared.AutoCAD.UI.HyCadRibbonBuilder.UnsubscribeSelfHealOnly();
            }
            catch { }

            try { HyCADTool.Shared.AutoCAD.UI.CuiMenuBuilder.UnloadOnExitOnly(); } catch { }
        }
