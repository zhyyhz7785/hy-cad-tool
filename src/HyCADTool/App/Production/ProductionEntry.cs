...
[assembly: ExtensionApplication(typeof(HyCADTool.App.Production.ProductionExtension))]
[assembly: CommandClass(typeof(HyCADTool.App.Production.ProductionCommandFacade))]
...
namespace HyCADTool.App.Production
{
    ...
    /// 内部直接走 <see cref="HyCADTool.App.Bootstrap.PluginInitializer"/>
    /// 与开发模式中 ReCall 反射调用的入口完全一致 → 业务初始化路径 100% 复用。
    /// </summary>
    public sealed class ProductionExtension : IExtensionApplication
    {
        private static HyCADTool.App.Bootstrap.PluginInitializer _initializer;

        public void Initialize()
        {
            try
            {
                _initializer = new HyCADTool.App.Bootstrap.PluginInitializer();
