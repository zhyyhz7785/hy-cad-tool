...
namespace HyCADTool.Features.Road.CrossSection.Views
{
    ...
    public partial class CrossSectionDrawPanel : UserControl
    {
        ...
        private void OnPickGeometryRequested(object sender, BandRowViewModel row)
        {
            ...
            try
            {
                var result = HyCADTool.Features.Road.CrossSection.Services.Workflows.RoadCsPickGeometryInteractor.PickAndExtract(row.Kind);
                if (result == null) return;
                row.Width = result.Width;
                row.CrossSlopePct = result.SlopePct;
                row.ElevationDiff = result.ElevationDiff;
                HyCADTool.Features.Road.CrossSection.Services.Workflows.RoadCsPickGeometryInteractor.DistributeThickness(row, result.ThicknessCm);
