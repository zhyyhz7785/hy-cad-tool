...
namespace HyCADTool.Features.Elevation
{
    ...
    public class Elevation3DCommand
    {
        ...
        /// 步骤2: 提取标高数据
        /// </summary>
        private Dictionary<Polyline, ElevationValue> ExtractElevationData(
            Editor ed,
            Database db,
            List<Polyline> polygons)
        {
            var elevationDict = new Dictionary<Polyline, ElevationValue>();
        ...
        private List<GeometryData> AnalyzeGeometry(
            List<Polyline> polygons,
            Dictionary<Polyline, ElevationValue> elevationDict)
        {
            var doc = Application.DocumentManager.MdiActiveDocument;
            var ed = doc.Editor;
            ...
            var polygon2DToElevation = new Dictionary<Polygon2D, ElevationValue>();
            ...
            foreach (var kvp in polygon2DToElevation)
            {
                ...
                foreach (var boundary in boundaryConditions)
                {
                    ...
                    {
                        outerElevation = ElevationValue.FromMeters(0.0); // 土壤边界默认地面标高
                    }
                    else
                    {
