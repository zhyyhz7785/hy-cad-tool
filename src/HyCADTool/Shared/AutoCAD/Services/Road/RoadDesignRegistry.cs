...
{
    /// <summary>
    /// 活动 RoadDesign 注册表：�?AutoCAD 文档名缓存内存中�?Domain 聚合根�?    ///
    /// 职责（决�?1 - 单文�?JSON / 决策 3 - 真实事件总线）：
    /// - �?AutoCAD MdiActiveDocument.Name 索引 <see cref="RoadDesign"/> 实例�?    /// - 切换文档 / 打开 DWG 时懒加载；关闭文档时清理�?    /// - 作为命令�?Save 前的读取入口�?c>RoadJsonExportService.SaveForDocument</c> 调用方先 TryGet 再写盘）�?    ///
    /// 线程模型：AutoCAD 命令线程顺序访问，文档事件（DocumentActivated / DocumentCreated）亦在主线程�?    /// 仍采�?<see cref="ConcurrentDictionary{TKey, TValue}"/> 防止未来并发扩展�?    /// </summary>
    public sealed class RoadDesignRegistry
    {
        ...
        /// <summary>
        /// 替换指定文档�?RoadDesign（通常来自 JSON 加载），并发�?<see cref="RoadDesignReloadedEvent"/>�?        /// </summary>
        ...
        /// <summary>
        /// 移除指定文档�?RoadDesign（文档关闭时调用）�?        /// </summary>
        ...
        /// <summary>
        /// �?<paramref name="oldKey"/> 下的 <see cref="RoadDesign"/> 迁移�?<paramref name="newKey"/>�?        ///
        /// 场景�?        /// - DWG 另存为（SAVEAS）后 <c>MdiActiveDocument.Name</c> 改变�?        ///   �?key 下的 design 成为孤儿（自动保存会写到老路径，命令�?<c>GetOrCreate</c> 出一个空壳）�?        /// - 打开"同一份道�?JSON 属于的另一�?DWG 副本"时，用户可能希望把内存里�?design 绑到�?DWG�?        ///
        /// 语义�?        /// - <paramref name="oldKey"/> 不存在：返回 false，不改动�?        /// - <paramref name="oldKey"/> �?<paramref name="newKey"/> 相等（忽略大小写）：返回 false�?        /// - <paramref name="newKey"/> 已有 design：为防止静默覆盖历史数据，返�?false；调用方需先决定策略（通常�?<see cref="Replace"/>）；
        /// - 否则：迁移并移除�?key，返�?true。不触发事件 —�?迁移是基础设施级操作，�?UI 不可见�?        /// </summary>
        ...
        /// <summary>
        /// 快照所有文档的 RoadDesign（诊�?/ 调试用）�?        /// </summary>
