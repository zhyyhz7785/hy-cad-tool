...
namespace HyCADTool.Presentation.Views.Road
{
    ...
    public partial class CrossSectionDrawPanel : UserControl
    {
        ...
        private void OnPickGeometryRequested(object sender, BandRowViewModel row)
        {
            ...
            try
            {
                var result = HyCADTool.Shared.AutoCAD.Workflows.Road.RoadCsPickGeometryInteractor.PickAndExtract(row.Kind);
                if (result == null) return;
                row.Width = result.Width;
                row.CrossSlopePct = result.SlopePct;
                row.ElevationDiff = result.ElevationDiff;
                HyCADTool.Shared.AutoCAD.Workflows.Road.RoadCsPickGeometryInteractor.DistributeThickness(row, result.ThicknessCm);
