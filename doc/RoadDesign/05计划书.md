...
| P1 Alignment + Profile | **50%** | Alignment 主线（`hyRoadA` 拾取 / `hyRoadAlnByPi` 导线法 / `hyRoadAlnStation` 桩号）已可用；Profile 仍是空白占位 |
...
- **已完成**：`Profile` 领域对象 + `hyRoadP` 只会「追加空 Profile」。
- **下一块砖**：地面线 EG（从高程点 / 等高线生成）、设计线 FG（多控制点 + 自动竖曲线）、纵断面窗口（WPF Canvas）。**整条主线尚未落地，属于最短板。**
...
- [ ] **B1 Profile 地面线 EG**：`hyRoadProfEG`（从高程点或等高线采样）。
- [ ] **B2 Profile 设计线 FG**：`hyRoadProfFG`（多控制点 + 自动竖曲线）。
- [ ] **B3 纵断面窗口**：WPF Canvas 内嵌，显示 EG + FG。
- [ ] **B4 Profile 桩号标高 `hyRoadProfLabel`**。
...
- **本周内**：启动 **B1（EG 地面线）** 的技术选型（从高程点 vs 等高线，先哪个）。
